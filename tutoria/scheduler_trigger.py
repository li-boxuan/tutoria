    ...
    for session in sessions:
        ...
        for record in records:
            if record.status == record.ONGOING:
                ...
                record.save()
                # print("set record (id: ", record.id, ") status from ONGOING to FINISHED")  # Ignore PycodestyleBear (E501)
                ...
                if session.tutor.tutor_type == session.tutor.PRIVATE_TUTOR:
                    ...
                    content = 'Please check on Tutoria, your session ' + \  # Ignore PycodestyleBear (E501)
                        str(session.id)
                    # Ignore PycodestyleBear (E501)
                    content += ' has finished and your wallet balance has changed from '
                    content += str(receiver.user.wallet_balance -  # Ignore PycodestyleBear (E501)
                                   amount) + ' to '
                    content += str(receiver.user.wallet_balance)
                    send_mail('Session Finished & Balance Change', content,  # Ignore PycodestyleBear (E501)
                              'noreply@hola-inc.top', [receiver.email], False)

                    # (todo) company receives commission
                    myTutor = User.objects.get(username='mytutors')
                    myTutor.wallet_balance += commission
                    myTutor.save()
                    # print("transaction id = ", transaction.id, "transfer tuition fee = ", amount, " to tutor = ", receiver)  # Ignore PycodestyleBear (E501)

                # send email to student (review reminder)
                content = 'Please check on Tutoria, your session ' + \  # Ignore PycodestyleBear (E501)
                    str(session.id)
                content += ' has finished and you can give a comment to the tutor at '  # Ignore PycodestyleBear (E501)
                content += 'http://127.0.0.1:8000/tutor/' + \  # Ignore PycodestyleBear (E501)
                    str(session.tutor.id) + '/review'
                send_mail('Session Finished, Please give your comment', content,
                          'noreply@hola-inc.top', [record.student.email], False)

    lock_time_bound = time + timedelta(days=1)
    print('lock_time_bound = ', lock_time_bound)
    sessions = Session.objects.filter(
        start_time__gt=time, start_time__lt=lock_time_bound)
    for session in sessions:
...
def run_range(start_time, end_time):
    start_time = parser.parse(start_time)  # Ignore PyFlakesBear
