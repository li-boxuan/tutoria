...
class ReviewView(LoginRequiredMixin, FormView):
    ...
    def can_review(self):
        student = User.objects.get(
            username=self.request.session['username']).student
        tutor = Tutor.objects.get(pk=self.kwargs['tutor_id'])
        finished_booking_list = BookingRecord.objects.filter(
            tutor=tutor, student=student, status='F')
    ...
    def form_valid(self, form):
        ...
        review.student = User.objects.get(
            username=self.request.session['username']).student
