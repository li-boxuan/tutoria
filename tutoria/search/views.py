...
class ResultView(ListView):
    ...
    tutor_type = 'ALL'
    # Only show tutor with available session in the coming 7 days?
    only_show_available = False

    def dispatch(self, request, *args, **kwargs):
        self.price_limit = list(Tutor.objects.all().aggregate(
            Max('hourly_rate')).values())[0]
    ...
    def get_queryset(self):
        ...
        if 'only_show_available' in self.request.GET:
            self.only_show_available = eval(
                self.request.GET['only_show_available'])
        ...
        if self.tutor_type != 'ALL':
            filtered_tutors = [
                t for t in filtered_tutors if t.tutor_type == self.tutor_type]
